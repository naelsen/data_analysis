...
data.qual = data[,1:5]
data.qual$Age = paste(lapply(data.qual$Age,change_age))
profils = data.frame(table(data.qual))
profils = profils[profils[,6]>0,]
profils = paste(profils[,1],profils[,2],profils[,3],profils[,4],profils[,5],sep="/")
data_df = data[,7:20]
col_name = colnames(data_df)
data_df = cbind(data_df,paste(data.qual[,1],data.qual[,2],data.qual[,3],data.qual[,4],data.qual[,5],sep="/"))
colnames(data_df) = c(col_name,"profil")
data_df=aggregate.data.frame(x=data_df[-15], by=list(data_df$profil), FUN=mean)
```

```{r}
data_df.dist = dist(data_df[,-1])
```
...
head(data.qual)
data_count_1 <- aggregate(data.quant[,2] ~ data.qual , data, mean)
data_count_1
```
