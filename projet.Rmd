...
cah.ward = hclust(DF.agg.dist, method="single")
...
for (i in 1:K) Means_groupes[i,] <- colSums(DF.tmp[groupes.cah==i,][,-1])/dim(DF.notes)[1]
Means_groupes
```
...
for (i in 1:K) Means_groupes[i,] <- colMeans(DF.tmp[kmeans.clusters$cluster==i,][,-1])
colnames(Means_groupes)=c(colnames(DF.notes[,-1]),"satisfied")
rownames(Means_groupes)= paste("Groupe",1:K,sep="_")
groupes.kmeans = kmeans.clusters$cluster
DF.tmp = data.frame(DF.notes,(DF[bool_profil,23]=="satisfied")*1)
for (i in 1:K) Means_groupes[i,] <- colMeans(DF.tmp[groupes.kmeans==i,][,-1])
Means_groupes
```
...
fviz_cluster(kmeans.clusters, data = DF.agg,
             palette = c("#0008FB", "#971556", "#E7B800","#69F006" , "#F00606","#06F0CD" , "#EE05C0"), 
             geom = "point",
             ellipse.type = "convex", 
             ggtheme = theme_bw(),
