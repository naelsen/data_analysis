...
barplot((tmp/rowSums(tmp))[,2],
         width=rowSums(tmp),
         col=rainbow(n=52,alpha=0.8,start=0,end=0.4),
...
```
On remarque sur le boxplot qu'il semble y avoir une corrélation entre les notes attribuées à Wifi et à la Faclité de reservation, nous allons maintenant le verifier.
...
```{r}
K=7
DF.agg.dist = dist(DF.agg, method = "maximum") # "manhattan" (L1), "euclidean" (L2), "maximum" (L.infini), "minkowski", p = truc (L.truc)
...
for (i in 1:K) Means_groupes[i,] <- colMeans(DF.tmp[DF.tmp$profil %in% rownames(data.frame(groupes.cah[groupes.cah==i])),][,-1])
Means_groupes
```
En divisant par le nombre d'individus total, on prend en compte le fait que les groupes ne sont pas de même taille. On a ces résultats qui sont différents, c'est une piste à exploiter.


Le clustering hiérarchique nous permet de determiner le nombre de groupes idéal : $K = 7$. Les notes attribuées selon les différentes groupes ne sont pas significativement différentes.
...
```{r}
Means_groupes <- matrix(NA, nrow=K, ncol=dim(DF.notes)[2])
kmeans.clusters = kmeans(DF.agg, centers =K, nstart = 500)
for (i in 1:K) Means_groupes[i,] <- colMeans(DF.tmp[DF.tmp$profil %in% rownames(data.frame(kmeans.clusters$cluster[kmeans.clusters$cluster==i])),][,-1])

colnames(Means_groupes)=c(colnames(DF.notes[,-1]),"satisfied")
rownames(Means_groupes)= paste("Groupe",1:K,sep="_")
groupes.kmeans = kmeans.clusters$cluster
DF.tmp = data.frame(DF.notes,(DF[bool_profil,23]=="satisfied")*1)
for (i in 1:K) Means_groupes[i,] <- colMeans(DF.tmp[DF.tmp$profil %in% rownames(data.frame(groupes.kmeans[groupes.kmeans==i])),][,-1])
...
```{r}
fviz_cluster(kmeans.clusters, data = DF.agg,
             palette = rainbow(K), 
             geom = "point",
             ellipse.type = "convex", 
             ggtheme = theme_bw(),
             main = "Kmeans : Représentation des clusters")
             #points( kmeans.clusters$centers))
```

On remarque que les clusters 2 et 6 sont assez proches par exemple, obeservons les moyennes des notes attribués par les individus: 


Notes des individus du groupe 2 : 
```{r,echo=FALSE}
Means_groupes[2,]
```
Notes des individus du groupe 6 : 
```{r,echo=FALSE}
Means_groupes[6,]
#summary(lm(Means_groupes[,2] ~ Means_groupes[,6]))
```

On peut voir que les moyennes des notes attribués par individus des deux clusters sont très proches
