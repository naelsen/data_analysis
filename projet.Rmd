...
score.tree.opt = sum((pred.class.tree.opt==DF.test[,23])*1)/length(pred)
score.tree.opt
```
### LDA / QDA
...
```{r}
# --- LDA
pred.class.lda <- predict(res.lda,  newdata = DF.test[,c(7:20,23)])$class
pred.prob.lda <- predict(res.lda,  newdata = DF.test[,c(7:20,23)])
...
pred.prob.qda <- predict(res.qda,  newdata = DF.test[,7:20])$prob
tab.qda <- table(DF.test$Satisfaction ,pred.class.qda)
score.qda = sum(pred.class.qda==DF.test[,23])/length(pred.class.qda)
score.qda
```
On lance une procédure de sélection de modèle avec une méthode de cross validation 3-fold. Le temps d'exécution de la méthode par défaut (10-fold) était trés long du à la taille du jeu de données. 

```{r}
res.stepwise.lda=stepclass(Satisfaction ~., data=DF.train[1:5000,c(7:20,23)] , method="lda", direction="backward", fold=3)
res.stepwise.qda=stepclass(Satisfaction ~., data=DF.train[1:5000,c(7:20,23)] , method="qda", direction="backward", fold=3)

res.stepwise.lda = lda(res.stepwise.lda$formula , data = DF.train[1:5000,c(7:20,23)] )
res.stepwise.qda = qda(res.stepwise.qda$formula , data = DF.train[1:5000,c(7:20,23)] )
```
...
score.stepwise.lda = sum(round(pred.stepwise.lda)==(DF.test[,c(7:20,23)]$Satisfaction =="satisfied")*1)/dim(DF.test[1:1000,c(7:20,23)])[1]
score.stepwise.lda
```

La séléction de modèle avec qda a un score de 85.1% 
```{r}
pred.prop.stepwise.qda <- predict(res.stepwise.qda,newdata=DF.test[1:1000,c(7:20,23)])$posterior[,2]
score.stepwise.qda = sum(round(pred.prop.stepwise.qda)==(DF.test[1:1000,c(7:20,23)]$Satisfaction =="satisfied")*1)/dim(DF.test[1:1000,c(7:20,23)])[1]
score.stepwise.qda
```
...
On obtient un score de 96.2% avec le Random Forest
```{r}
...
score.RF=sum((pred.class.RF==DF.test[,23])*1)/length(pred)
score.RF
```
...
ROC.stepwise.lda  <- roc(DF.test[1:1000,]$Satisfaction, pred.prop.stepwise.lda)
ROC.stepwise.qda  <- roc(DF.test[1:1000,]$Satisfaction, pred.prop.stepwise.qda)
...
legend("bottomright",col = 1:5, lwd=2,c("lda","qda","Tree","Tree opt","RF"))
plot(ROC.stepwise.qda,print.auc=TRUE,add = TRUE , print.auc.y = 0.4, col = 2)
plot(ROC.tree,print.auc=TRUE,add = TRUE , print.auc.y = 0.3, col = 3)
plot(Roc.tree.opt,print.auc=TRUE,add = TRUE , print.auc.y = 0.2, col = 4)
plot(ROC.RF,print.auc=TRUE,add = TRUE , print.auc.y = 0.1, col = 5)
