...
```
L'arbre de décision prédit correctement la satisfaction des voyageurs avec 88% de précision binaire.

```{r}
pred.cart = predict(res.tree, newdata=DF.test[,-23], type="class")
sum((pred.cart == DF.test[,23])*1)/length(pred.cart)
```

```{r}
arbre=rpart(DF.train$Satisfaction~.,DF.train,control=rpart.control(minsplit = 50 ,cp=0))
...
```{r}
DF.test
res.lda <- lda(DF.train$Satisfaction~.,data=DF.train)
res.qda <- qda(DF.train$Satisfaction~.,data=DF.train)
```
